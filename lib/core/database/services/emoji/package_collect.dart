...
import '../base.dart';
import 'package:beaver/core/database/tables/emoji/package_collect.dart';
...
class EmojiPackageCollectService extends BaseService {
  ...
  Future<void> create(Map<String, dynamic> req) async {
    await db.into(db.emojiPackageCollectTable).insert(EmojiPackageCollectTableCompanion(
  ...
  Future<void> batchCreate(Map<String, dynamic> req) async {
    ...
    for (final collectData in collects) {
      await db.into(db.emojiPackageCollectTable).insert(
  ...
  Future<List<dynamic>> getCollectsByUserId(Map<String, dynamic> req) async {
    ...
    final result = await db.select(db.emojiPackageCollectTable).where((t) => t.userId.equals(userId)).get();
  ...
  Future<dynamic> getCollectById(Map<String, dynamic> req) async {
    ...
    final result = await db.select(db.emojiPackageCollectTable).where((t) => t.packageCollectId.equals(id)).get();
  ...
  Future<void> delete(Map<String, dynamic> req) async {
    final id = req['id'] as String;
    await db.delete(db.emojiPackageCollectTable).where((t) => t.packageCollectId.equals(id)).go();
  }
}
