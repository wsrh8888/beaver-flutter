...
final _logger = Logger('business-notification-event');
...
class NotificationEventBusiness {
  ...
  Future<void> handleTableUpdates(String eventId) async {
    _logger.info({'text': '收到通知事件表更新', 'data': {'eventId': eventId}});
    await syncEventsByIds([eventId]);
  }

  Future<void> syncEventsByIds(List<String> eventIds) async {
    _logger.info({'text': '开始按ID同步通知事件', 'data': {'count': eventIds.length}});
    ...
    try {
      ...
      _logger.warn({'text': '按ID同步通知事件异常', 'data': {'error': e.toString()}});
      rethrow;
    }
  }
}
